...
int main(int argc, char** argv)
{
	...
	// Load file
	fseek(fm1v, 0, SEEK_END);
	m1v.file_size = ftell(fm1v);
	fseek(fm1v, 0, SEEK_SET);
	m1v.file_buffer = (uint8_t*)malloc(m1v.file_size);
	fread(m1v.file_buffer, m1v.file_size, 1, fm1v);
	fclose(fm1v);
	...
	while(finished)
	{
		...
		switch(stream_id)
		{
			...
			default:
				if(m1v_is_slice(stream_id)) fprintf(flog, "Slice size: %x\n", m1v.slice_size);
				else finished = 0;
		}
	}
